                    ...
                    "update" => {
                        if let (Some(channel), Some(result)) = (&response.channel, &response.result) {
                            let market_event = match channel.as_str() {
                                ...
                                "spot.order_book" | "spot.order_book_update" => {
                            ...
                            if let Err(e) = self.sender.send(market_event) {
...
impl DataProvider for GateProvider {
                ...
                match message {
                    Ok(msg) => {
                        if let Err(e) = provider_clone.handle_message(msg).await {
                            log::error!("Error handling Gate message: {}", e);
                        }
                    }
                    Err(e) => {
    ...
    async fn unsubscribe(&mut self, symbols: Vec<Symbol>) -> Result<()> {
        ...
        for symbol in &symbols {
            ...
            let channels = vec!["spot.tickers", "spot.order_book_update", "spot.trades"];
...
mod tests {
    ...
    fn test_gate_provider_creation() {
        ...
        let result = GateProvider::new(config);
        assert!(result.is_ok());
    ...
    fn test_symbol_conversion() {
        ...
        let (provider, _receiver) = GateProvider::new(config).unwrap();
